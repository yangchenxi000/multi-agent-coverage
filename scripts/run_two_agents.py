"""Run Experiment 2: two-agent baseline vs ours."""
...
def main() -> None:
    baseline_cfg = yaml.safe_load((ROOT / "configs" / "two_agents_baseline.yaml").read_text(encoding="utf-8"))
    ours_cfg = yaml.safe_load((ROOT / "configs" / "two_agents_ours.yaml").read_text(encoding="utf-8"))

    set_seed(int(ours_cfg["seed"]))
    out_dir = ROOT / "outputs" / "two_agents"
    run_compare_two_configs(baseline_cfg, ours_cfg, out_dir, comparison_name="baseline_vs_ours_2agents")

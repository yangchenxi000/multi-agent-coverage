"""Run one simple Mars scene: two-agent baseline vs ours."""
...
def main() -> None:
    baseline_cfg = yaml.safe_load((ROOT / "configs" / "two_agents_mars_easy_baseline.yaml").read_text(encoding="utf-8"))
    ours_cfg = yaml.safe_load((ROOT / "configs" / "two_agents_mars_easy_ours.yaml").read_text(encoding="utf-8"))
    ...
    # Requested 2-agent snapshot schedule for figures.
    for c in (baseline_cfg, ours_cfg):
        c["optimization"]["iterations"] = 220
        c["optimization"]["snapshot_iters"] = [0, 20, 40, 60, 80, 100, 150, 220]

    set_seed(int(ours_cfg["seed"]))
    out_dir = ROOT / "outputs" / "two_agents_mars_easy"
    run_compare_two_configs(baseline_cfg, ours_cfg, out_dir, comparison_name="baseline_vs_ours_2agents_mars_easy")
